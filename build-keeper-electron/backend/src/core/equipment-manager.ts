...
import type { EquipmentSet, PreparedEquipmentSet } from '../types';
...
export function getUniqueName(baseName: string, existingNames: string[]): string {
  ...
  for (const name of existingNames) {
    if (name === prefix) {
      ...
      if (nameMatch) {
        maxNum = Math.max(maxNum, parseInt(nameMatch[1], 10));
      }
    }
  }
 ...
 */
export function createSet(data: Partial<EquipmentSet>, existingNames: string[]): EquipmentSet {
  return {
    ...data,
    id: uuidv4(),
